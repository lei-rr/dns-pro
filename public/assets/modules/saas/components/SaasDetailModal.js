    ...
    <a-modal :open="open" @update:open="v => $emit('update:open', v)" title="自定义主机名详情" :footer="null" width="760px" destroy-on-close>
      ...
      <template v-if="hostname">
        ...
        <a-descriptions bordered size="small" :column="1" style="margin-bottom: 16px">
          ...
          <a-descriptions-item v-if="hostname.custom_origin_server" label="回源服务器">{{ hostname.custom_origin_server }}</a-descriptions-item>
          <a-descriptions-item v-if="hostname.ssl?.settings?.min_tls_version" label="最低 TLS 版本">{{ minTlsLabel(hostname.ssl?.settings?.min_tls_version) }}</a-descriptions-item>
          <a-descriptions-item v-if="hostname.ssl?.issuer" label="证书颁发">{{ hostname.ssl.issuer }}</a-descriptions-item>
          <a-descriptions-item v-if="hostname.ssl?.expires_on" label="证书到期">{{ formatDate(hostname.ssl?.expires_on) }}</a-descriptions-item>
